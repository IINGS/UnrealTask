...
void ASPTPawn::Look(const FInputActionValue& Value)
{
    ...
    // Pawn의 움직임이나 회전에는 영향을 주지 않고, 오직 스프링암의 상대 회전을 업데이트합니다.
    FRotator CurrentRotation = SpringArmComponent->GetRelativeRotation();

    // Yaw(좌우 회전): 입력 X에 비례하여 변경 (원하는 속도로 곱해줍니다.)
    ...
    // 스프링암의 상대 회전 업데이트 (이로 인해 카메라만 회전합니다.)
